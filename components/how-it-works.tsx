      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8 relative">
          ...
          {data.steps?.map((step: any, index: number) => (
            <div key={step._key} className="group relative z-10 flex flex-col items-center bg-white border-2 border-slate-200 rounded-2xl p-8 shadow-md hover:shadow-xl hover:-translate-y-3 transition-all duration-500">
              ...
              <div className={`mt-auto w-full aspect-square rounded-xl bg-slate-50 flex items-center justify-center border border-slate-100 overflow-hidden ${step.isIconOnly ? 'p-4' : 'p-0'}`}>
                {step.visual && (
                  <img 
                    src={urlFor(step.visual).url()} 
                    alt={`Visual ${step.title}`} 
                    className={`w-full h-full object-contain transition-transform duration-500 transform ${step.isIconOnly ? 'opacity-80 scale-100' : 'mix-blend-multiply scale-110'} group-hover:scale-125`} 
                  />
                )}
              </div>
