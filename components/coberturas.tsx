...
interface CoberturasProps {
  ...
  complementaryCoverages?: CoverageItem[];
  additionalCoverages?: CoverageItem[];
}
...
export function Coberturas({
  ...
  const fila2 = complementaryCoverages || [
    {
  ...
  const fila3 = additionalCoverages || [
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center max-w-3xl mx-auto mb-16">
          <span className="inline-block px-3 py-1 rounded-sm bg-primary/10 text-primary text-[10px] font-bold uppercase tracking-[0.2em] mb-6 border border-primary/20">
            {heroSubtitle || "Coberturas"}
          </span>
          <h2 className="text-3xl sm:text-4xl lg:text-[2.75rem] font-extrabold text-slate-900 leading-[1.15] text-balance mb-6">
            {heroTitle || "Todo lo que necesitas para manejar"} <span className="text-primary">{heroTitleHighlight || "tranquilo."}</span>
          </h2>
          <p className="text-lg text-slate-500 font-medium leading-relaxed">
            {heroDescription || "Elige las coberturas que se adaptan a ti. Nosotros comparamos las mejores opciones entre 6 aseguradoras."}
          </p>
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-8 mb-8">
          {fila1.map((item, idx) => (
            <div key={idx} className="bg-white border-2 border-slate-100 rounded-2xl p-8 hover:border-primary/20 hover:shadow-xl transition-all duration-300 ease-in-out hover:-translate-y-1 text-center flex flex-col items-center group shadow-md">
              {item.icon ? (
                <img src={typeof item.icon === 'string' ? item.icon : urlFor(item.icon).url()} alt={item.title} className="w-40 h-40 object-contain mb-6 transition-transform duration-500 group-hover:scale-110" />
              ) : (
                <div className="w-40 h-40 mb-6 bg-slate-100 rounded-full flex items-center justify-center text-slate-400 text-xs">Sin imagen</div>
              )}
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-16">
          {fila3.map((item, idx) => (
            <div key={idx} className="bg-white border-2 border-slate-100 rounded-2xl p-8 hover:border-primary/20 hover:shadow-xl transition-all duration-300 ease-in-out hover:-translate-y-1 text-center flex flex-col items-center group shadow-md">
              {item.icon ? (
                <img src={typeof item.icon === 'string' ? item.icon : urlFor(item.icon).url()} alt={item.title} className="w-32 h-32 object-contain mb-6 transition-transform duration-500 group-hover:scale-110" />
              ) : (
                <div className="w-32 h-32 mb-6 bg-slate-100 rounded-full flex items-center justify-center text-slate-400 text-xs">Sin imagen</div>
              )}
